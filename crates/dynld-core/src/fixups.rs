            ...
            FixupKind::Bind {
                ...
                match *ordinal {
                    ...
                    BIND_SPECIAL_DYLIB_WEAK_LOOKUP => unsafe {
                        ...
                        let sym = symbols
                            .iter()
                            .find(|sym| sym.name.as_bytes() == symbol_name.as_bytes())
                    ...
                    _ => {
                        if let Some(shared_cache) = dyld_shared_cache {
                            let cstr = symbol_name.as_str();
                            let dylib = dylibs[*ordinal as usize - 1];

                            let symaddr = shared_cache
                                .symbol_resolve(dylib.as_bytes(), cstr.as_bytes())?
                                .unwrap();
                            ...
                            return Err(
                                "could not fixup chains as there was not shared cache provided",
                            );
                        }
                    }
                };
