...
/// Extended mapping entry that also carries slide info. Used via
/// `mapping_with_slide_offset` / `mapping_with_slide_count`.
#[repr(C)]
#[derive(Debug, Clone, Copy, Pread)]
...
/// Lighter-weight image entry used by tools that only need text-segment info.
#[repr(C)]
#[derive(Debug, Clone, Copy, Pread)]
pub struct dyld_cache_image_text_info {
    /// LC_UUID of the embedded image.
    pub uuid: [u8; 16],
    /// Unslid VM address of the start of `__TEXT`.
    pub load_address: u64,
    /// Byte size of the `__TEXT` segment.
...
/// Older, pre-`file_suffix` form of [`dyld_subcache_entry`]. Some caches in
/// the wild still use this shape.
#[repr(C)]
#[derive(Debug, Clone, Copy, Pread)]
pub struct dyld_subcache_entry_v1 {
    /// LC_UUID of the subcache file.
    pub uuid: [u8; 16],
    /// Offset of this subcache from the main cache's base address (in unslid VM space).
    pub cache_vm_offset: u64,
...
/// `cache_type` values.
pub const DYLD_SHARED_CACHE_TYPE_DEVELOPMENT: u64 = 0;
pub const DYLD_SHARED_CACHE_TYPE_PRODUCTION: u64 = 1;
pub const DYLD_SHARED_CACHE_TYPE_UNIVERSAL: u64 = 2;

/// Bits for [`dyld_cache_mapping_and_slide_info::flags`].
