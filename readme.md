...
The responsibility that `dyld` carries is respectable. No program, apart from `dyld`, can run without it. It knows precisely how to handle Mach-O in all its forms. This project will unfortunately never reach such a level. Especially since it's difficult to find information on this topic, there's little available on what's required to implement a dynamic linker on Apple platforms. This project aims to help those who want to run programs manually without using `dlopen`, `dlsym`, `execve`, etc.
