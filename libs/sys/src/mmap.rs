...
use crate::{
    ...
    syscalls::{syscall2, syscall3, syscall6},
};
...
unsafe fn munmap_internal(addr: *mut u8, len: usize) -> Result<(), c_int> {
    br(unsafe { syscall2(SYS_MUNMAP, addr as usize, len) }, |_| ())
}
...
unsafe fn mprotect_internal(addr: *mut u8, len: usize, prot: c_int) -> Result<(), c_int> {
    br(
        unsafe { syscall3(SYS_MPROTECT, addr as usize, len, prot as u32 as usize) },
        |_| (),
    )
}
